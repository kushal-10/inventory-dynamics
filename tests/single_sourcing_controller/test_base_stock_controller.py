...
def test_base_stock_controller_fit_and_cost(
    ...
    expected_z_star,
    expected_avg_cost,
    base_stock_controller,
    single_sourcing_model_0,
    single_sourcing_model_2,
):
    model = single_sourcing_model_0 if lead_time == 0 else single_sourcing_model_2
    base_stock_controller.fit(model, seed=42)
    assert (
    ...
    avg_cost = base_stock_controller.get_average_cost(
        model, sourcing_periods=1000, seed=42
    )
    assert (
...
@pytest.mark.parametrize("lead_time", [0, 2])
def test_base_stock_controller_simulation(
    lead_time, base_stock_controller, single_sourcing_model_0, single_sourcing_model_2
):
    model = single_sourcing_model_0 if lead_time == 0 else single_sourcing_model_2
    base_stock_controller.fit(model, seed=42)

    past_inventories, past_orders = base_stock_controller.simulate(
        model, sourcing_periods=100
    )
    assert (
...
@pytest.mark.parametrize(
    ...
    [
        (0, 10, torch.tensor([[0.0]])),
        (2, 10, torch.tensor([[1.0]])),
    ],
)
