...
class DualSourcingModel(torch.nn.Module):
    ...
    def simulate(self):
        ...
        qra = self.previous_qr[self.current_timestep - self.lr]
        qea = self.previous_qe[self.current_timestep - self.le]
    ...
    def calculate_cost(self, qe, qr, current_demand, current_inventory):
        return  self.ce * qe + self.cr * qr + self.h * torch.relu(current_inventory - current_demand) \
              + self.b * torch.relu(current_demand - current_inventory)

    def replay_step(self, previous_inventory, current_demand, qra, qea, qr, qe):

        current_inventory = previous_inventory + qra + qea

        c_i = self.ce * qe + self.cr * qr + self.h * torch.relu(current_inventory - current_demand) \
              + self.b * torch.relu(current_demand - current_inventory)
        current_inventory = current_inventory - current_demand

        return c_i, current_inventory

    def reset(self, minibatch_size=16, seed=None):
