...
class NNCDIController(DualSourcingController):
    ...
    def __init__(self,
        ...
        self.lr = lr
        sf_init = torch.rand([1])*10
        ss_init = torch.rand([1])*10
        qbars_init = torch.rand([1])*10
        self.sf = torch.nn.Parameter(sf_init, requires_grad=True)
        self.ss = torch.nn.Parameter(ss_init, requires_grad=True)
        self.qbars = torch.nn.Parameter(qbars_init, requires_grad=True)
        self.allow_rounding_correction = allow_rounding_correction
    ...
    def forward(self,
        ...
        # minibatch samples
        n_samples = current_inventory.shape[0]

        # net current inventory
        I_t = current_inventory[:, -1:, 0:1]

        # calculate the future arrived orders
        future_qr = past_regular_orders[:, -self.lr:, 0:1]
        future_cumul_qr = future_qr.cumsum(1)
        # net_inventories = torch.zeros([n_samples, self.lr + 1, 1]) + I_t
        # net_inventories[:, 1:, :] += future_cumul_qr
        net_inventories =I_t + future_cumul_qr

        current_net_after_repl = net_inventories[:, 0:1, :]
        net_pos_after_repl = net_inventories[:, -1:, :]


        sf = torch.relu(self.sf - 1)
        qe = torch.relu(sf - current_net_after_repl)
        ss = torch.relu(self.ss - 1)
        base_stock = torch.relu(ss - net_pos_after_repl)
        qbars = torch.relu(self.qbars - 1)
        qr = qbars - torch.relu(qbars.detach() - base_stock)

        if self.allow_rounding_correction:
            qe = qe - torch.frac(qe).clone().detach()
            qr = qr - torch.frac(qr).clone().detach()

        return qr.squeeze(1), qe.squeeze(1), None
...
class GatedSensitivityController(DualSourcingController):
    ...
    def __init__(self,
        ...
        self.lr = lr
        self.inputs = lr + 1
        self.linear_qr = torch.nn.Linear(self.inputs, self.inputs)
        self.linear_qe = torch.nn.Linear(self.inputs, self.inputs)
    ...
    def forward(self,
        ...
        future_qr = past_regular_orders[:, -self.lr:, 0:1]
        future_cumul_qr = future_qr.cumsum(1)
        net_inventories = future_cumul_qr + I_t

        scores_qr = torch.softmax(self.linear_qr(net_inventories.squeeze(-1), -1))
        scores_qe = torch.softmax(self.linear_qe(net_inventories.squeeze(-1), -1))

        qr = scores_qr.unsqueeze(-1)*net_inventories
        qe = scores_qe.unsqueeze(-1)*net_inventories
