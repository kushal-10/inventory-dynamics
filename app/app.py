...
with tab1:
    ...
    with c1:
        ...
        if demand_type == 'Uniform':
            with st.form(key='uniform_demand'):
                high = (1 << 53) - 1
                low = st.number_input("Minimum demand", value=0, placeholder="Type an integer...", step=1,
                                      format='%i', max_value=high)
                high = st.number_input("Maximum demand", value=4, placeholder="Type an integer...", step=1,
                ...
                with cg2:
                    if submitted and high >= low:
                        st.success(
                            "Successfully generated uniform demand within range: [" + str(np.floor(low)) + ", " + str(
                                np.floor(high)) + "]")
                        st.session_state['training'] = 0
                ...
                with cgg2:
                    if submitted2:
                        try:
                            ...
                            st.success('File successfuly uploaded and contains ' + str(df.shape[0]) + ' demand points:')
                            # st.table(df)
    ...
    with c2:
        if st.session_state['demand_generator'] is not None:
            ...
            with c2c1:
                fig = px.line(y=all_demands).update_layout(xaxis_title="Periods", yaxis_title="Demand",
...
with tab2:
    ...
    with st.form(key="Dual Sourcing Model"):
        ...
        with c1t2:
            ...
            batch_size = np.int32(
                st.number_input('Minibatch/sample size for demand trajectories:', value=16, min_value=0,
                                format='%i', step=1))
            init_inventory = np.int32(
                st.number_input('Initial inventory:', value=6, min_value=0, format='%i', step=1))
        ...
        with c2t2:
            ...
            holding_cost = np.int32(
                st.number_input('Holding cost:', value=5, min_value=0, format='%i', step=1))
            shortage_cost = np.int32(
                st.number_input('Shortage cost:', value=495, min_value=0, format='%i', step=1))
...
with tab3:
    ...
    with c1:
        ...
        layer_sizes = st.text_area(label="Layer sizes: A comma seperated list of integers,"
                                   ...
                                   value='5,5,5')
        sourcing_periods = st.number_input('Number of training sourcing periods:', value=100, min_value=1, format='%i',
                                           step=1)
        validation_sourcing_periods = st.number_input('Number of validation sourcing periods:', value=10, min_value=1,
                                                      format='%i', step=1)
    ...
    with c2:
        ...
        if st.session_state['demand_controller'] is not None and st.session_state['dual_sourcing_model'] is not None:
            st.info('Graph plot for minibatch size: 4, click top right corner to enlrage!')
...
with tab4:
    if st.session_state['demand_controller'] is not None and st.session_state['dual_sourcing_model'] is not None and \
            st.session_state['demand_generator'] is not None:
        if os.path.exists("runs/dual_sourcing_model") and st.session_state['training'] > 0:
            try:
                ...
                with t4c2:
                    ...
                    fig = px.line(df_past).update_layout(xaxis_title='Periods', yaxis_title='# Units', title='Sample Optimization Trajectory')
                    st.plotly_chart(fig)
            except Exception:
                st.warning("No available model, please make sure you submit the previous steps!")
    else:
        if st.session_state['demand_generator'] is None:
            st.warning("No demand generator is chosen trained for current configuration, please define one.")
        if st.session_state['dual_sourcing_model'] is None:
            st.warning("No sourcing model is for current configuration, please define one.")
        if st.session_state['demand_controller'] is None:
            st.warning("No model chosen trained for current configuration, please go to previous step and fit a model.")
