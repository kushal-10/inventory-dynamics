...
The iterations are as follows:
 ...
 - Apply the update:
.. math::
   J_{k+1}(\mathbf{s}) = \min\limits_{\mathbf{Q} \in \mathcal{D}_{\mathbf{Q}}} \left\{ c_{\rm e}q^{\rm e} + \sum\limits_{\mathbf{s}' \in \mathcal{S}} P(\mathbf{s}' | \mathbf{s}, \mathbf{Q})(f(\mathbf{s}')+J_{k+1}(\mathbf{s})) \right\}`, for all :math:`\mathbf{s} \in \mathcal{S}`
